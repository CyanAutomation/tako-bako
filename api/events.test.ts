...
describe("outcome proxy", () => {
  ...
  it("forwards only anonymous, bounded puzzle outcomes", async () => {
    ...
    expect(upstream).toHaveBeenCalledWith("https://yokaiba.scheimann.workers.dev/v1/events", expect.objectContaining({ body: JSON.stringify({ event: "puzzle_completed", templateId: "tournament-order-v2", assessedDifficultyLevel: 2, elapsedMs: 1000, smartMarkingEnabled: true }) }));
