...
describe("puzzle UI sections", () => {
  it("composes the puzzle heading and board toolbar from shared primitives", () => {
    ...
    const toolbar = renderBoardToolbar({ marked: 2, total: 16, assist: true, undoDisabled: false, redoDisabled: true, checkDisabled: true });
    ...
    expect(toolbar).toContain('aria-label="Auto-eliminate is on"');
    expect(toolbar).toContain('aria-pressed="true"');
    expect(toolbar).toContain('icon--sparkle');
  });
  ...
  it("composes workspace, clues, and settings without page-level markup", () => {
    ...
    expect(renderPuzzleSettings({ seed: "dojo-day", settingsOpen: true })).not.toContain('assist-toggle');
  });
});
