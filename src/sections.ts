...
export function renderCurriculum({ completed, currentCourseId, showHeading = true }: { completed: ReadonlySet<string>; currentCourseId: CourseId; showHeading?: boolean }): string {
  return `<section class="curriculum"${showHeading ? ' aria-labelledby="curriculum-title"' : ""}>${showHeading ? '<div class="curriculum-heading"><div><p class="eyebrow">Your route through the dojo</p><h2 id="curriculum-title">Puzzle Challenge</h2></div><p>Complete each level to unlock the next.</p></div>' : ""}<div class="curriculum-tiers">${TIERS.map(tier => {
    ...
    return `<section class="curriculum-tier curriculum-tier--${tier}" aria-label="${title} levels"><div class="curriculum-tier-heading"><h3>${title}</h3>${renderInfoDisclosure({ id: `${tier}-info`, label: `More information about ${title}`, content: `<p>${description}</p>` })}</div><ol>${tierCourses.map((course, index) => {
      ...
      return `<li class="course course--${state}">${renderButton({ label: String(course.level), ariaLabel: `${course.label}, ${state}`, variant: course.id === currentCourseId ? "primary" : "secondary", disabled: !unlocked, data: { course: course.id } })}<span class="course-state" aria-hidden="true">${stateLabel}</span></li>`;
    }).join("")}</ol></section>`;
  }).join("")}</div></section>`;
}
...
export function renderBoardToolbar({ matches, total, undoDisabled, checkDisabled, assist }: { matches: number; total: number; undoDisabled: boolean; checkDisabled: boolean; assist: boolean }): string {
  const checkHint = checkDisabled ? "Place one match in every row and column to check." : "Your board is ready to check.";
  const assistLabel = `Fill rule-outs: ${assist ? "on" : "off"}`;
...
export function renderGridWorkspace({ categories, activeGridId, toolbar = "", grids }: { categories: { id: string; label: string }[]; activeGridId: string; toolbar?: string; grids: string }): string {
  return `<div class="board-workspace">${toolbar}${renderTabs(categories, activeGridId)}<p class="legend"><span class="legend-mark yes">✓</span> match <span class="legend-mark no">×</span> rule out <span class="legend-mark unknown"></span> unmarked <span class="legend-tip">Select a square to cycle its mark · use arrow keys to move.</span></p><section class="grids" aria-label="Logic grids">${grids}</section></div>`;
}
