...
export function renderBoardToolbar({ matches, total, undoDisabled, checkDisabled, hintDisabled, assist }: { matches: number; total: number; undoDisabled: boolean; checkDisabled: boolean; hintDisabled: boolean; assist: boolean }): string {
...
export function renderCluePanel({ clues, activeCategory, cluesOpen, usedClueIds, clueFilter = "all" }: { clues: { id: string; text: string }[]; activeCategory: Category; cluesOpen: boolean; usedClueIds: ReadonlySet<string>; clueFilter?: ClueFilter }): string {
  ...
  const filterControls = (["all", "remaining", "used"] as const).map(filter => renderButton({ label: filter === "all" ? "All" : filter === "remaining" ? "To review" : "Used", pressed: clueFilter === filter, data: { clueFilter: filter } })).join("");
  return renderPanel({
